...
export default function BHUTANForm() {
  ...
  useEffect(() => {
    ...
    if (showSuccessModal && countdown > 0) {
      ...
      router.push('/singapore-malasiya-tour-packages-from-bangalore-lp/thank-you');
    }
  ...
  const sendToWeb3Forms = async (data) => {
    try {
      const emailPayload = {
        ...
        subject: 'New SINGAPORE Tour Inquiry',
        message: `
New SINGAPORE Tour Inquiry Details:
  ...
  const sendToMakeWebhook = async (data) => {
    try {
      const webhookPayload = {
        ...
        source: 'SINGAPORE Tour Form',
        leadSource: 'SINGAPORE Tour Landing Page'
      };
  ...
  const sendToTeleCRM = async (values) => {
    try {
      const fields = {
        ...
        SAG: values['sag'] || '',
        DOM: values['dom'] || '',
        INTL: values['international'] || 'SINGAPORE',
      ...
      const actions = [
        {
          type: 'SYSTEM_NOTE',
          text: `Lead Source: Google Ads
Destination: ${values['international'] || 'SINGAPORE'}
