  ...
  const sendToWeb3Forms = async (data) => {
    try {
      const emailPayload = {
        ...
        subject: 'New BHUTAN Tour Inquiry',
        message: `
New BHUTAN Tour Inquiry Details:
...
Submission Time: ${data.submissionTime}
        `,
        reply_to: data['your-email'] || 'noreply@saishishirtour.in',
        from_name: 'BHUTAN Tour Form',
      };
  ...
  const sendToMakeWebhook = async (data) => {
    try {
      const webhookPayload = {
        ...
        source: 'BHUTAN Tour Form',
        leadSource: 'BHUTAN Tour Landing Page'
      };
  ...
  const sendToTeleCRM = async (values) => {
    try {
      const fields = {
        ...
        SAG: values['sag'] || '',
        DOM: values['dom'] || '',
        INTL: values['international'] || 'BHUTAN',
...
Destination: ${values['international'] || 'BHUTAN'}
